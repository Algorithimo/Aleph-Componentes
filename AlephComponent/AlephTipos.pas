...
type
  TREmMargins = class( TPersistent )
    ...
    published
      property AMarginBase : TFontBase
        read GetFontBase
        write setFontBase;
      property Top : TREmSize
        read FTop
        write SetTop;
      property Left : TREmSize
        read FLeft
        write SetLeft;
      property Right : TREmSize
        read FRight
        write SetRight;
      property Bottom : TREmSize
        read FBottom
        write SetBottom;
  end;
  ...
  TAlephTipo = class( TPersistent )
    ...
    published
      property PHeight : Double
        read FPHeight
        write SetPHeight;
      property PWidth : Double
        read FPWidth
        write SetPWidth;
      property ActivePWidth : Boolean
        read FActiveWidth
        write SetActiveWidth;
      property ActivePHeight : Boolean
        read FActiveHeight
        write SetActiveHeight;
    public
      procedure Resize;
...
procedure TAlephTipo.ResizeComponent( Sender : TObject );
  ...
  begin
    ...
    if Assigned( FControl.Parent )
    then
    begin
      // Verifica se o Parent é um TForm
      if FControl.Parent is TForm
      then
      begin
      ...
      else if FControl.Parent is TControl
      then
      begin
      ...
      begin
        ...
        try
          if FActiveWidth and ( FPWidth > 0 )
          then
          begin
            FControl.Width := RoundTo( ParentWidth * FPWidth / 100, - 2 );
          end;
          if FActiveHeight and ( FPHeight > 0 )
          then
          begin
...
procedure TAlephTipo.Resize;
  var
    ParentWidth, ParentHeight, CurrentWidthPercent,
      CurrentHeightPercent : Double;
  begin
    if csDesigning in FControl.ComponentState
    then
    Begin
      if Assigned( FControl.Parent )
      then
      begin
        ...
        begin
          ...
          then
            CurrentWidthPercent :=
              RoundTo( FControl.Width / ParentWidth * 100, - 1 )
          else
          ...
          then
            CurrentHeightPercent :=
              RoundTo( FControl.Height / ParentHeight * 100, - 1 )
          else
...
constructor TREmMargins.Create( AControl : TControl );
  begin
    ...
    if not Assigned( FFontBase )
    then
    begin
      FFontBase := TFontBase.Create( 12 );
    end;

    if Assigned( FFontBase )
    then
    begin
